...
- [x] workflow 默认 refs 改由 `suite-lock.yml` 解析并校验完整 SHA（`e900a0a`）。
- [x] 替代 run `31267714152` 成功；008 审计已升级为 PASS。
- [ ] 2026-08-14 前按[处置 Runbook](../../docs/runbooks/render-ghcr-credential-rotation.md)
  轮换 Render 私有 GHCR 拉取凭据或下线服务。

## 后续需先决策

- [x] 接受 [`ADR-003`](adr/ADR-003-M3-BFF-identity-boundary.md) 并解锁
  [`009-M3-AUTH`](dispatches/009-M3-AUTH.md)。
